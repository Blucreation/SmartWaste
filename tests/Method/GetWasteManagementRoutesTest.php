...
class GetWasteManagementRoutesTest extends TestCase
{
    /**
    ...
    {
        new GetWasteManagementRoutes();
    }

    public function testInit()
    {
        $uri = new GetWasteManagementRoutes(ConvertEntity::ID, ConvertEntity::ID);

        $this->assertTrue(true);

        $url = SmartWasteRoutes::toUrlRelative($uri);

        $this->assertEquals('12345/projects/12345/waste-management-routes', $url);
    }
    ...
    public function testSuccess()
    {
        ...
        $val = $ret[0]->wasteManagementRouteName ?? null;
        $this->assertEquals('Reuse', $val);
    }
}
