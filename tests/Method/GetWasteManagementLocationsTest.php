...
class GetWasteManagementLocationsTest extends TestCase
{
    /**
    ...
    {
        new GetWasteManagementLocations();
    }

    public function testInit()
    {
        $uri = new GetWasteManagementLocations(ConvertEntity::ID, ConvertEntity::ID);

        $this->assertTrue(true);

        $url = SmartWasteRoutes::toUrlRelative($uri);

        $this->assertEquals('12345/projects/12345/waste-management-locations', $url);
    }
    ...
    public function testSuccess()
    {
        ...
        $val = $ret[0]->wasteManagementLocationName ?? null;
        $this->assertEquals('On site', $val);
    }
}
